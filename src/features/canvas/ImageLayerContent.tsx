...
export const ImageLayerContent = ({
  ...
  >
      {!object.imageConfig || !object.imageConfig.url ? (
        <ImagePlaceholder>No image selected</ImagePlaceholder>
      ) : (
        <>
          <CanvasImage
            key={`${object.id}-${imageRetryTokens[object.id] ?? 0}-${object.imageConfig.url}`}
            data-testid={`canvas-image-${object.id}`}
            src={object.imageConfig.url}
            alt="Canvas image"
            draggable={false}
            $fitMode={object.imageConfig?.size ? DEFAULT_IMAGE_FIT_MODE : DEFAULT_IMAGE_FIT_MODE}
